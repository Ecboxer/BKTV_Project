...
# tidy data for clevelad plot and map
flight_state <- flight %>%
  group_by(dest_state) %>%
  dplyr::summarize(sum_n = n(), sum_arr = sum(arr_delay), sum_dep = sum(dep_delay), sum_diff = sum(diff)) %>%
  mutate(mean_arr = sum_arr/sum_n, mean_dep = sum_dep/sum_n, mean_diff = sum_diff/sum_n)
...
ggplot(flight_state) +
  ...
  labs(title = "Delay Difference between Depature and Arrival Delays over States", x = "Mean Difference in Mintutes", y = "", color = '')
...
diff_map$title = "Delay Time Difference of Flights in the fourth quarter of the year 2017"
diff_map$show_labels = FALSE
diff_map$set_num_colors(1)
diff_map$ggplot_scale = scale_fill_viridis(name = 'Delay Time Difference in Minutes')
diff_map$render()
```
