...
Flights seem to be evenly distributed over all days except Saturdays. This somewhat makes sense, as travellers with flights on Saturdays are more likely to be on vacations - and they are more likely to depart on Friday for their destination. How do delays relate to days of the week?
...
Evidently, delays are very similar over each day so there's no significant observation to be made. Can we learn anything by looking at negative delays?
...
Plotting the scatterplot of distance in correlation with the Delays in the Departure, helps us determine the following:  
...
The delay distribution varies very little among the top 5 airports. The delay distribution between popular and unpopular airports does not differ as much as we hoped, so there's no insight to be made here. 
...
Airline popularity also doesn't seem to play a factor in departure delay. 
...
We can construct a variable called diff_delay that measures the difference between departure delay and arrival delay. This will allow us to examine whether a departure delay is likely to be amended for. 
...
arr_map$title = "Arrival Delay of Flights in the fourth quarter of 2017"
...
The interesting pattern between flight departure times and arrival times enables us to explore the reasons behind it. The first question we asked was whether travel distance influences flight delays. Though there is no true travelling distance for each flight, the destination state of each flight is a rough representation of distance. We found that neither depature delay nor arrival delay associate with distance, represented by state locations. For example, both flights that flew to New Jersey and to Puerto Rico had high average departure delay and arrival delay, but New Jersey is much closer to New York than Puerto Rico. We surprisingly found that average departure delay is higher than average arrival delay, whether traveling distance is far from or close to New York. This implies that pilots make up time in the air when flights leave later than expected. Even when flights leave earlier than the expected depture time in Alaska, pilots still make up time during flights.
...
difference_map$title = "Delay Time Difference of Flights in the fourth quarter of 2017"
...
Since we found that pilots usually make up time in the air, we next asked 'Which states had the greatest make-up time?' To our surprise, make-up time does not relate to travel distance, which is represented by state location. For instance, flights from New York to Maryland, Minnesota, New York, South Carolina and Arizona had similar make-up times. However, New York is much more closer to Maryland and South Carolina than to Minnesota and Arizona. Several states showed interesting make-up times; U.S. Pacific Trust Territories and Possessions with the highest and Hawaii and Alaska with the lowest. This could be a route for further exploration.
