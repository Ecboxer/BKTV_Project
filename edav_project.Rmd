...
Having chosen our topic, we steered our focus towards finding a dataset that can answer the above mentioned questions for us. As our source for data, we used the website of the [United States Department of Transportation](https://transtats.bts.gov/databases.asp?Mode_ID=1&Mode_Desc=Aviation&Subject_ID2=0). Of course, based on our goal, we interacted only with the portion of their collected data library that deals with the topic of Aviation.
...
/- Day of the Week
/- Time of Departure
Yiannis State and 3 scatters/- Distance
/- Destination Airport
/- Carrier
...
We initiate our Exploratory Data Analysis by examining the Delays in the Departure time for every day of the month. The following scatterplot graph helps us determine whether a relationship can be infered?
...
Clearly, we can observe that during the Christmas Holidays the Departure Delays show an increase in duration, thus suggesting that the consumers spend a lot more time waiting in the airports to depart for their final destination. The same conclusion can be derived for the weekend following Thanksgiving (23 of November, Thursday), which again constitutes an important holiday for the US. People, wait a great amount of time to return to their homes on Sunday instead of leaving a day earlier that the delays are reduced. Finally, we can onserve that during certain days of the week, like Tuesdays or Wednesdays, we have reduced Departure Delay times which is expected since the main reasons for travelling during the week are mostly business related.
...
Does distance have any specific characteristics and how does the Distance relate to the Delays in Departure?
...
Plotting the scatterplot of the Distance in correlation with the Delays in the Departure, helps us determine the following. 

* The different values of the delays recorded for a specific distance, seem to form a line.  
* Also, there exist specific intervals of time within a day that the delays accumulate and should be our main focus. 
...
Upon further investigation of the distance variable, we produced a plot of the unique values of the distances traveled to reach each State destination.
...
Additionally, we create a graph of the unique distances that each US carrier claims to have covered. 
...
Based on the two graphs presented, we were able to observe that the values of the distance variable are a lot more discrete than they were suppose to. Logically speaking, we should have a great variety of values since it is extremely rare for two planes to cover the exact same distance going back or forth to the same destination-origin point. By going back to the source of our data, we were able to conclude that the distance provided by our dataset is an actual approximation of the shortest distance between any two points on the surface of a sphere. Therefore, any two flights with the same start and end points have the same distances. This can be easily verified from the table below.
...
The delay distribution varies very little between the top 5 airports. The delay distribution between popular and unpopular airports does not differ as much as we hoped, so there's no insight to be made here. 
...
df$pop_carrier <- ifelse(df$carrier %in% top_carriers$carrier, "Popular", 0)
df$pop_carrier <- ifelse(df$carrier %in% last_carriers$carrier, "Unpopular", df$pop_carrier)
head(subset(df, df$pop_carrier != 0), 5)
```

```{r carrier boxplots}
ggplot(subset(df, df$pop_carrier != 0), aes(x=factor(carrier), y=log(dep_delay))) + geom_boxplot(varwidth=TRUE, fill = '#774184') +
  ggtitle('Distribution of Positive Delay in the busiest and least busiest Airlines') +
  facet_grid(. ~ pop_carrier) +
  xlab("Airline") +
  ylab("log(Delay in Minutes)")
```

The popularity of the airline also doesn't seem to play a factor on the departure delay. 
...
### Destination Airport
```{r airport popularity boxplots}
ggplot(subset(df, df$pop_dest != 0 & df$pos_makeup == 'Positive'), aes(x=factor(dest), y=log(dep_delay))) + geom_boxplot(varwidth=TRUE, fill = '#774184') +
  ggtitle('Distribution of Makeup Delay in the busiest and least busy Airports') +
  facet_grid(. ~ pop_dest) +
  xlab("Airport") +
  ylab("log(Positive Makeup Delay in Minutes)")
```
...
ggplot(flight_state) +
  ...
  labs(title = "Delay Time of Flights over States", x = "Mean Delay in Mintutes", y = "", color = '') +
...
arr_map$title = "Arrival Delay of Flights in the fourth quarter of the year 2017"
...
The interesting pattern between flights' depature time and arrival time enabled us to explore the reasons behind it.  The first question was whether the destination state influence on the flights' time performance because some states are far away from New York City, but some are close, and some have large population density, but some have small pouplation density. We found that neither the depature delay or the arrival delay associate with the states. The states with high arrival delay time and low arrival delay time distribute on each region of the United States. The top five states with high arrival delay are New Jersey, Puerto Rico, U.S. Virgin Islands, Hawaii and New York, and there were no information about the flights from New York City to Delaware. The bottom five states with low arrival delay are Virginia, Maryland, Utah, South Carolina and U.S. Pacific Trust Territories and Possessions.
However, we surprisingly found that all the average depature time was higher than the average arrival time in the relevant state, whatever they are the location and the poulation of the states. This implies that the piolets usually make up time during the flight driving when the flights leave later than then expected depature time. Even when the flights leave earlier than the expected depture time in Alaska, the piolts still make up time during their flight driving. Thus, we investigated the make up time during the pilots' driving to each states.
...
difference_map$title = "Delay Time Difference of Flights in the fourth quarter of the year 2017"
difference_map$show_labels = FALSE
difference_map$set_num_colors(1)
difference_map$ggplot_scale = scale_fill_viridis(name = 'Delay Time Difference in Minutes')
difference_map$render()
```

To our surprise, the flights to the northwest states had lower make up time than other states, including Washing, Oregon, Idaho, Montana, Wyoming, North Dakota and South Dakota. This implies that the piolts do not need to make up much time when they fly to these states. The states with high time up and low time up distribute on the other regions of the United States. In addition to the northwest states, Alaska, Hawaii, U.S. Virgin Islands and Mississippi also have low make up time. The top five high make up time appear in flights flew to U.S. Pacific Trust Terries and Possessions, Maryland, Minnesota, New York and New Hampshire, which means the piolts make up more time when they fly to these states than others.
