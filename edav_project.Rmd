...
The dominant missing data pattern is for observations to be lacking only cancellation code. Infrequently, observations are missing departure and arrival times, but we felt that all observations missing either of those values could be removed without losing information about delay durations.

```{r cancellation missing values}
df %>% filter(is.na(canc_code) & cancelled == 1) %>% nrow()
```

There are no cases in which cancellation code is missing but the data tells us that the flight was actually cancelled. That would have been a clear signal that some observations were incorrectly entered, as it is we will assume that the data on cancellations is accurate.
...
=======
  xlab('canc_code')
>>>>>>> cd0b7f67173d3b9a19ec694881b2aab54add836d
```
...
We initiate our Exploratory Data Analysis by examining the Departure Delay for every day of the month. The following scatterplot graph helps us determine whether a relationship can be infered.
...
<<<<<<< HEAD
Clearly we can observe that during the Christmas holidays, departure delay shows an increase in duration, thus suggesting that passengers spend a lot more time waiting in airports to depart for their final destination. The same conclusion can be derived for the weekend following Thanksgiving (Thursday the 23 of November), which again constitutes an important holiday for the US. People wait a great amount of time to return to their homes on Sunday instead of leaving a day earlier when delays are reduced. Finally, we can observe that during certain days of the week, like Tuesdays or Wednesdays, we have reduced Departure Delay times. This is expected since the main reasons for travelling during the week are business related.
=======
Clearly we can observe that during the Christmas holidays, Departure Delay shows an increase in duration, thus suggesting that passengers spend a lot more time waiting in the airports to depart for their final destination. The same conclusion can be derived for the weekend following Thanksgiving (23 of November, Thursday), which again constitutes an important holiday for the US. Departure delays during the Thanksgiving period are at a high on Sunday, when most people are returning home. Finally, we can observe that during certain days of the week, like Tuesdays or Wednesdays, have reduced Departure delay times which is expected since the main reasons for travelling during the week are mostly business related.
>>>>>>> cd0b7f67173d3b9a19ec694881b2aab54add836d
...
```	Flights seem to be evenly distributed over all days except Saturdays. This somewhat makes sense, as travellers with flights on Saturdays are probably on vacation - and are consequently more likely to depart for their destination on Friday. How do delays relate to days of the week?
...
ggplot(df_backup, aes(x = dep_delay, y = distance)) +
    ...
    ylab("Distance (Miles)") +
    ggtitle("Departure Delays in correlation with the travelled Distance") +
    labs(caption = "Source: Final Project Dataset")
```

<<<<<<< HEAD
The scatterplot of distance in correlation with departure delay helps us determine the following:  
* Different values of delay recorded for a specific distance seem to form a line.  
* There exist specific intervals of time within a day that delays accumulate and these should be our main focus. 
=======
...
<<<<<<< HEAD
    ...
    labs(caption = "Source: Final Project Dataset")
=======
    ggtitle("Departure Delay in relation to the Flight Distance",
          subtitle = "Limited to 12 hours delays - early departures") +
    labs(caption = "Source: Final Project Datatset")
>>>>>>> cd0b7f67173d3b9a19ec694881b2aab54add836d
```
...
=======
    ggtitle("Departure Delay in relation to the Flight Distance",
            subtitle = "Limited to 3 hours delays - early departures") +
    labs(caption = "Source: Final Project Datatset")
>>>>>>> cd0b7f67173d3b9a19ec694881b2aab54add836d
```

Now, we can clearly bound the times of the departure delays that we are going to examine between a maximum of 12 hours delay and a maximum of 2 hours early departure. This follows from the fact that a 3 hour delay or early departure interval (latter graph) does not constitute a clear representation of the phenomenon that we are exploring. On the contrary, a 12 hours delay interval is more than enough to give us the full picture and when combined with the 2 hours early departure barrier allow us to have a clear picture of the situation at hand. 

Further investigating the distance variable, we produce a plot of the unique values of distance traveled to reach each State.
...
<<<<<<< HEAD
    ggtitle("Distance in correlation with the State Destination",
...
=======
    ggtitle("Distance in relation to the State Destination",
            subtitle = "Unique Distances travelled to reach corresponding State destination") +
    labs(caption = "Source: Final Project Datatset")
>>>>>>> cd0b7f67173d3b9a19ec694881b2aab54add836d
```

Additionally, we create a graph of the unique distances that each Airline claims to have covered. 
...
ggplot(distance_carrier_data, aes(x = carrier, y = distance)) +
    ...
    ylab("Distance (Miles)") +
<<<<<<< HEAD
    ggtitle("Distance in correlation with the US carrier") +
    labs(caption = "Source: Final Project Dataset")
=======
...
top_airports <- head(top_airports, 5)
last_airports <- df %>% group_by(dest) %>% summarize(num_flights = n())
last_airports <- subset(last_airports, last_airports$num_flights > 100)
last_airports <- last_airports[order(last_airports$num_flights), ]
last_airports <- head(last_airports, 5)
df$pop_dest <- ifelse(df$dest %in% top_airports$dest, "Popular", 0)
df$pop_dest <- ifelse(df$dest %in% last_airports$dest, "Unpopular", df$pop_dest)
head(subset(df, df$pop_dest != 0), 5)
```

We can also visualize the distribution of departure delay at our top airports in order to see if flights are more likely to be delayed when headed to more popular destinations.

```{r airport boxplots}
ggplot(subset(df, df$pop_dest != 0), aes(x=factor(dest), y=log(dep_delay))) + geom_boxplot(varwidth=TRUE, fill = '#774184') +
  ggtitle('Distribution of Positive Delay in the busiest and least busiest Airports') +
  facet_grid(. ~ pop_dest) +
  xlab("Airport") +
  ylab("log(Delay in Minutes)") +
  labs(caption = "Source: Final Project Dataset")
...
=======
Are certain airlines more likely to cause a delay? 
>>>>>>> cd0b7f67173d3b9a19ec694881b2aab54add836d
...
The five most frequent airlines are the following:
- WN: Southwest Airlines
- DL: Delta Airlines
- AA: American Airlines
- OO: Skywest Airlines
...
The five 'least frequent' airlines are the following:
- VX: Virgin America
- HA: Hawaiian Airlines
- F9: Frontier Airlines
- NK: Spirit Airlines
...
A reminder that this is only in regards to domestic flights departing the State of New York.
>>>>>>> cd0b7f67173d3b9a19ec694881b2aab54add836d
...
How do departure delay and arrival delay generally relate to each other?
...
We can construct a variable called diff_delay that indicates the difference between departure delay and arrival delay. This will allow us to examine whether a departure delay is likely to be ammended for. 
...
ggplot(subset(df, df$pop_carrier != 0 & df$pos_makeup == 'Positive'), aes(x=factor(carrier), y=log(dep_delay))) + geom_boxplot(varwidth=TRUE, fill = '#774184') +
  ggtitle('Distribution of Positive Makeup Delay in the most and least busy Airlines') +
  facet_grid(. ~ pop_carrier) +
  xlab("Airline") +
...
=======
Apparently, the size of an airline doesn't seem to have an impact on its ability to make-up for a delay. 

### Destination State

We wish to explore any patterns relating departure time and arrival time. Does the distance between the State of New York and the destination State influence a flights' time performance? What about the destination State's population density? 
>>>>>>> cd0b7f67173d3b9a19ec694881b2aab54add836d

```{r destination state arrival time}
# Data frame for the cleveland plot
df$diff <- df$dep_delay - df$arr_delay
flight <- df
...
# Tidy data for Cleveland plot
flight_state <- flight %>%
  group_by(dest_state) %>%
  dplyr::summarize(sum_n = n(), sum_arr = sum(arr_delay), sum_dep = sum(dep_delay), sum_diff = sum(diff)) %>%
  mutate(mean_arr = sum_arr/sum_n, mean_dep = sum_dep/sum_n, mean_diff = sum_diff/sum_n)
# Cleveland plot of arrival time and departure time
...
ggplot(flight_state) +
  ...
  labs(title = "Flight Delay over States", x = "Mean Delay in Minutes", y = "", color = '') +
...
flight_state2 <- df %>%
  group_by(dest_state) %>%
  dplyr::summarize(sum_n = n(), sum_arr = sum(arr_delay), sum_diff = sum(diff)) %>%
  mutate(mean_arr = sum_arr/sum_n, mean_diff = sum_diff/sum_n)
# State map of arrival time
flight_state_map <- subset(flight_state2, select = c('dest_state', 'mean_arr'))
names(flight_state_map)[1] <- paste('region')
names(flight_state_map)[2] <- paste('value')
region_code <- c("1", "2", "3", "4", "5", 
                 ...
                 "91", "92", "93")
region_name <- c("alaska", "hawaii", "puerto rico", "us virgin islands", "us pacific trust territories and possessions", 
...
region_code2 <- "31"
region_name2 <- "delaware"
flight_state_map$region[flight_state_map$region == region_code] <- region_name
flight_state_map$region[flight_state_map$region == region_code2] <- region_name2
arr_map <- StateChoropleth$new(flight_state_map)
<<<<<<< HEAD
arr_map$title = "Arrival Delay of Flights in the fourth quarter 2017"
=======
arr_map$title = "Arrival Delay of Flights"
>>>>>>> cd0b7f67173d3b9a19ec694881b2aab54add836d
arr_map$show_labels = FALSE
arr_map$set_num_colors(1)
arr_map$ggplot_scale = scale_fill_viridis(name = 'Mean Arrival Delay in Minutes')
arr_map$render()
```
...
We surprisingly found the average departure delay is higher than the average arrival delay in all states, no matter their location or population density. This implies that pilots can make up time during flights after delayed departures. 
=======

According to our graphs above, neither departure nor arrival delay are strongly associated with the destination State. The five States with the highest arrival delay are New Jersey, Puerto Rico, the U.S Virgin Islands, Hawaii and New York. The five States with the lowest arrival delay are Virginia, Maryland, Utah, South Carolina and the U.S Pacific Trust Territories and Possessions.

However, we surprisingly found the average departure delay is higher than the average arrival delay in all states, no matter the location or the population density. This seems to imply that pilots can often make-up time during the flight for delayed departures. 
...
<<<<<<< HEAD
  labs(title = "Mean Difference between Depature and Arrival Delay over States", x = "Mean Difference in Mintutes", y = "", color = '') +
  labs(caption = "Source: Final Project Dataset")
=======
  labs(title = "Mean Difference between Departure and Arrival Delay", x = "Mean Difference in Minutes", y = "", color = '')
>>>>>>> cd0b7f67173d3b9a19ec694881b2aab54add836d

# state map of time difference
diff_map <- subset(flight_state2, select = c('dest_state', 'mean_diff'))
names(diff_map)[1] <- paste('region')
names(diff_map)[2] <- paste('value')
region_code <- c("1", "2", "3", "4", "5", 
                 "11", "12", "13", "14", "15", "16", 
                 "21", "22", "23", 
                 "33", "34", "35", "36", "37", "38", "39", 
...
region_code2 <- "31"
region_name2 <- "delaware"
diff_map$region[diff_map$region == region_code] <- region_name
diff_map$region[diff_map$region == region_code2] <- region_name2

difference_map <- StateChoropleth$new(diff_map)
<<<<<<< HEAD
difference_map$title = "Delay Time Difference of Flights in the fourth quarter 2017"
=======
difference_map$title = "Difference between Departure and Arrival Delay"
>>>>>>> cd0b7f67173d3b9a19ec694881b2aab54add836d
difference_map$show_labels = FALSE
difference_map$set_num_colors(1)
difference_map$ggplot_scale = scale_fill_viridis(name = 'Difference in Minutes')
difference_map$render()
```

<<<<<<< HEAD
To our surprise, flights to the Northwestern states (Washington, Oregon, Idaho, Montana, Wyoming, North Dakota and South Dakota) had a lower delay difference than did others. This contradicts our hypothesis that pilots can make up for time lost during longer flights. There doesn't appear to be a relationship between destination and the difference between arrival and departure delay. In addition to the Northwest states, Alaska, Hawaii, U.S. Virgin Islands and Mississippi also have a low make-up delay.
=======
To our surprise, the flights to the Northwestern States (Washington, Oregon, Idaho, Montana, Wyoming, North Dakota and South Dakota) had a lower delay difference than others. This seems to reject our hypothesis that pilots can make up for time lost during longer flights. There doesn't appear to be a relationship between the destination and the difference between arrival and departure delay.  In addition to the Northwest States, Alaska, Hawaii, U.S. Virgin Islands and Mississippi also have a low make up delay.
...
[__Here__](https://bl.ocks.org/yannistze/a0a46f2740c8916dc71fc4da111995d1) is a scatterplot showing the distribution of the departure delays or early departures based on the day of the month. We used a sample of 20% from the whole dataset for better response times and a clear view of the results. Hovering over the name of the Month and then Clicking it will display an enlarged view of the distribution for the corresponding month. The scatterplot allows the user to acquire an accurate glimpse of how the delays and early departures are distributed within the different days of the month and maybe plan his or her departure date accordingly.

## Departure Delays and US Carriers

[__Here__](https://bl.ocks.org/yannistze/f6679d566e5c1d8ceabab043535dfcac) is a chart of boxplots that show the distribution of the departure delays based on the US carrier that fulfills the flight. Taking into account the width of the boxplots, we can infer which carrier fulfills the most flights. Also, when Clicking upon the boxplot of the carrier that interests the viewer, he or she can see the distribution of the departure delays of that carrier, as well as the distribution of the early departed flights. As a result, the viewer can compare the airline carriers and choose according to his or her needs.  
>>>>>>> cd0b7f67173d3b9a19ec694881b2aab54add836d
...
```
<<<<<<< HEAD
It appears that there's generally a positive linear relationship between the two variables. The higher is departure delay, the higher is arrival delay. There are some exceptions, and it's important to note that a large number of flights have high arrival delays despite having almost no departure delay. We hypothesize that this occurs when the destination landing strip is too busy. Pilots are unable to make a landing at the scheduled time. However, a plane can't fly aimlessly for long. Is this more common for certain airlines or certain airports?

## Make-up Delay and State
```{r main choropleth}
difference_map <- StateChoropleth$new(diff_map)
difference_map$title = "Average Difference between Departure and Arrival Delay"
difference_map$show_labels = FALSE
difference_map$set_num_colors(1)
difference_map$ggplot_scale = scale_fill_viridis(name = 'Difference in Minutes')
difference_map$render()
...
The Northwestern states; Washington, Oregon, Idaho, Montana, Wyoming, North Dakota and South Dakota have the lowest difference in separture and arrival delay. Again, this contradicts our hypothesis that longer flights allow a pilot to make-up for departure delays. The state with the highest mean difference in delays is New York, the complete opposite of what we hypothesized. Why short flights more often experience large differences in delay? 

# Conclusion

In this study, we used airline on-time performance data from the United States Department of Transportation to explore the performance of airlines departing New York in the fourth quarter of 2017. We analyzed departure time in relation to: flight date, distance, destination and carrier. We found that departure delays were higher during the holiday season, Thanksgiving and Christmas, and flights were more likely to depart early than late. In out data, the distance feature did not capture real-world distance travelled, but was an "as the crow flies" measurement of distance between origin and destination airports. For the most part, neither departure nor arrival delay were strongly associated with destination state. However, the Northwestern states experienced unusually small differences in departure and arrival delays. The carrier feature was not found to have a great impact on delay. After analyzing the flights' departure delays, we moved on to their arrival performance. There was a rough linear relationship between departure delay and arrival delay. Interestingly,  many flights arrived at their destination later than expected even when departing on time. Furthermore, the arrival delay  was not related to flight distance, reflected by destination state. Several airports showed higher arrival delays, such as Newark Liberty International Airport and San Francisco International Airport.  
=======

Flights overall appear to have a roughly linear relationship with arrival delay, debunking the common theory that pilots make up for time lost. The evident vertical line indicates that flights with no departure delay are often delayed post take-off. We hypothesize that this occurs due to busy landing strips where flights are forced to fly around the destination airport until a strip vacates. However, a plane can't fly aimlessly for long implying that there must be other underlying causes.
...
The North-Western States, Washington, Oregon, Idaho, Montana, Wyoming, North Dakota and South Dakota, have the lowest difference in Departure and Arrival Delay. This, again, contradicts our hypothesist that longer flights present more opportunities for a pilot to make-up for the departure delay. The State with the highest mean difference in delays is New York, completely opposite to what we originally thought. Why are short flights more likely to reduce the delay? 
>>>>>>> cd0b7f67173d3b9a19ec694881b2aab54add836d

Limited information and rough calculations from this dataset restricted us in our investigations. In this dataset, distance is calculated as the distance between two airports, not true  distance travelled. We are interested in the relationship between flight departure and arrival performance and it would be useful to know true distance from airplane meters. We are also curious about the number of passengers each airplane carried, which would add a new dimension to our analysis of flight performance. Another feature of our study was the relationship between departure delay and arrival delay. We are interested in analyzing more data to see whether this is a causal relationship. In addition, there were a number of flights which arrived late with on time or even early departures. The reasons behind this pattern could prompt further investigations.  

<<<<<<< HEAD
We did not analyze several interesting features in this dataset. The 'NA' value, which we filtered in departure and arrival delay variables, represents cancelled flights. Some reasons for cancellations were recorded in the data. It would be interesting to analyze the situations of cancellations and explore their reasons. We only focused on flights departing New York, with the largest city in the US. We would expect a spike in travel during the fourth quarter of the year with two major holidays. It would be interesting to analyze data from the whole dataset, 1987 to present, and flights departing from other cities, such as Washington D.C., San Francisco and Los Angeles. We could confirm our finding that flights are usually delayed longer during the holiday seasons and may also be able to make new discoveries which we did not obtain from our analysis, on states or destination airports.
=======
In this study, we used Airline On-Time Performance Data from the United States Department of Transportation to explore the on-time performances of airlines departed from New York City in the fourth quarter of the year of 2017. We analyzed departure time from the following aspects, the date, the distance, the states and the carrier. We found that the departure delay time was higher during the holiday season, Thanksgiving and Christmas and the flights were more likely to depart earlier than later (?). Conclusion of the relationship between distance and delay&state (?) Conclusion of the relationship between carrier and delay. After analyzing the flights' departure time performance, we moved on to their arrival time performance. There was a rough linear relationship between the departure time and arrival time. Another interesting discovery is many flights arrived at their destination later than the expected time even they departure on time. Further, the arrival delay time was not related to the flights' distance, reflected by the states, but several airports showed higher arrival delay performance, such as Newark Liberty International Airport and San Francisco International Airport. (?)
The limited information and rough calculation from this dataset limited us to do more further investigations. In this dataset, the distance is calculated as the distance between two airports, not the real traveling distance. We are interested in the relationship between the flights' departure and arrival time performance and the real traveling distance reflected by the airplane meters. We are also passionate to know more information about the number of passengers each airplane carried, and thus we can add a new dimension, the capacity of airline to our analysis of flights' time performance. Another in-dept study based on our results is the relationship between departure delay and arrival delay. We are interested in to analyze more data to see whether it is a causal relationship. In addition, there were certain number of flights arrived late with on time or even early departure, the reasons behind this pattern showed in our analysis attracted us to do more investigations.
We did not analyze several interesting features in this dataset due to the lack of time. The 'NA' value, which we filtered in the departure and arrival delay variables represents the cancelled planes. Some reasons for the cancellation were recorded in the online data. It will be interesting to analyze the situations of cancellations and explore the reasons. We only focused on the flights departed from New York City, a big city with great people transportation at the fourth quarter of the year 2017. It will be interesting to analyze the data from the whole dataset from xxx to present and the airplanes departed from big cities, such as Boston, Washington D.C., San Francisco and Los Angeles. We may can confirm our finding that the flights usually delay longer during the holiday seasons and may also conclude some new discoveries which we did not obtain from our analysis, such as the dimension of states and airports. 
