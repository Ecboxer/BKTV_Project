...
Looking at the weekdays:
```{r Distribution of Flights}
...
```{r}
ggplot(df, aes(x=weekday, y=dep_delay)) + geom_boxplot(varwidth=TRUE, fill = '#1FA088') +
...
```{r}
ggplot(subset(df, df$dep_delay >0), aes(x=weekday, y=log(dep_delay))) + geom_boxplot(varwidth=TRUE, fill = '#1FA088') +
...
ggplot(df, aes(x=weekday, y=log(abs(dep_delay)))) + geom_boxplot(aes(fill=factor(pos_delay)), varwidth=TRUE) +
  ...
  scale_fill_manual(values=c('#fce640', '#1FA088'))
```
