...
distance_state_data <- distance_state_data %>% mutate( state = states_recode[dest_state])

ggplot(distance_state_data, aes(x = factor(state), y = distance)) +
...
ggplot(flight_state) +
  geom_point(aes(mean_arr, dest_state, color = '#774184'), size = 3) +
  geom_point(aes(mean_dep, dest_state, color='#4E8441'), size = 3) +
  labs(title = "Delay Time of Flights over States", x = "Mean Delay in Mintutes", y = "", color = '') +
  theme(legend.position = 'bottom') +
  scale_color_hue(labels = c('Arrival Delay', 'Departure Delay'))
...
state_choropleth(flight_state_map,
                 title = "Arrival Delay of Flights in the fourth quarter of the year 2017",
                 legend = 'Mean Arrival Delay in Minutes')
```

The interesting pattern between flights' depature time and arrival time enables us to explore the reasons behind it.  The first question we asked was whether the distance of traveling influence on the flights' time performance. Though there is no real travling distance for each flight, the destination state of each flight can roughly represent their travling distance. We found that neither the depature delay or the arrival delay associate with the distance of traveling, represented by the locations of states. For exmaple, both flights flew to New Jersey and Puerto Rico had the high average deapture dealy and arrival dealy, but New Jersey is much closer to New York City than Puerto Rico. However, we surprisingly found that the average depature time is higher than the average arrival time, whatever the traveling distance is far from or close to New York City. This implies that the piolets usually make up time during the flight driving when the flights leave later than then expected depature time. Even when the flights leave earlier than the expected depture time in Alaska, the piolts still make up time during their flight driving.
...
state_choropleth(diff_map,
                 title = "Delay Time Difference of Flights in the fourth quarter of the year 2017",
                 legend = 'Delay Time Difference in Minutes')
```

Since we found that  the pilots usually make up time during their drivings, we next asked the question that the flights to which states had the most make up time. To our surprise, the make up time does not relate to the distance of traveling, which is represented by the states. For instance, the flights flew from New York City to Maryland, Minnesota, New York, South Carolina and Arizona had similar make up time, but the distance from New York City are much more closer to Maryland and New York to South Carolina, Minnesota and Arizona. Several states showed interesting make up time, U.S. Pacfic Trust Territories and Possessions with the highest and Hawaii and Alaska with the lowest, which need further exploration.
...
### Destination State
```{r destination state cleveland plot}
# data frame for this chuck
df$diff <- df$dep_delay - df$arr_delay
flight2 <- df
flight2$dest_state <- factor(flight2$dest_state)
region_code <- c("1", "2", "3", "4", "5", "11", "12", "13", "14", "15", "16", "21", "22", "23", "31", "33", "34", "35", "36", "37", "38", "39", "41", "42", "43", "44", "45", "51", "52", "53", "54", "61", "62", "63", "64", "65", "66", "67", "71", "72", "73", "74", "81", "82", "83", "84", "85", "86", "87", "88", "91", "92", "93", "31")
region_name <- c("Alaska", "Hawaii", "Puerto Rico", "U.S. Virgin Islands", "U.S. Pacific Trust Territories and Possessions", "Connecticut", "Maine", "Massachusetts", "New Hampshire", "Rhode Island", "Vermont", "New Jersey", "New York", "Pennsylvania", "Delware", "Florida", "Georgia", "Maryland", "North Carolina", "South Carolina", "Virginia", "West Virginia", "Illinois", "Indiana", "Michigan", "Ohio", "Wisconsin", "Alabama", "Kentucky", "Mississippi", "Tennessee", "Iowa", "Kansas", "Minnesota", "Missouri", "Nebraska", "North Dakota", "South Dakota", "Arkansas", "Louisiana", "Oklahoma", "Texas", "Arizona", "Colorado", "Idaho", "Montana", "Nevada", "New Mexico", "Utah", "Wyoming", "California", "Oregon", "Washington", "Delaware")
flight2$dest_state <- factor(flight2$dest_state, region_code, region_name)
# tidy data for cleveladn plot
flight_state <- flight2 %>%
  group_by(dest_state) %>%
  dplyr::summarize(sum_n = n(), sum_arr = sum(arr_delay), sum_dep = sum(dep_delay), sum_diff = sum(diff)) %>%
  mutate(mean_arr = sum_arr/sum_n, mean_dep = sum_dep/sum_n, mean_diff = sum_diff/sum_n)
# cleveland plot of arrival time and depature time
flight_state$dest_state <- reorder(flight_state$dest_state, flight_state$mean_arr)
ggplot(flight_state) +
  geom_point(aes(mean_arr, dest_state), color = 'red') +
  geom_point(aes(mean_dep, dest_state), color='blue')
# cleveland plot of time difference
flight_state$dest_state <- reorder(flight_state$dest_state, flight_state$mean_diff)
ggplot(flight_state) +
  geom_point(aes(mean_diff, dest_state), color = 'green') 
```

```{r destination state map}
library(choroplethr)
# tidy data for map
flight_state <- df %>%
  group_by(dest_state) %>%
...
# state map of arrival time
flight_state_map <- subset(flight_state, select = c('dest_state', 'mean_arr'))
names(flight_state_map)[1] <- paste('region')
names(flight_state_map)[2] <- paste('value')
region_code <- c("1", "2", "3", "4", "5", 
                 "11", "12", "13", "14", "15", "16", 
...
# state map of time difference
diff_map <- subset(flight_state, select = c('dest_state', 'mean_diff'))
names(diff_map)[1] <- paste('region')
names(diff_map)[2] <- paste('value')
region_code <- c("1", "2", "3", "4", "5", 
                 "11", "12", "13", "14", "15", "16", 
